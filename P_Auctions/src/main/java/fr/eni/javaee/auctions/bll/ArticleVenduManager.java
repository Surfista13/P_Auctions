...
public class ArticleVenduManager {
	...
	// select tous les articles vendus par nom d'article (pas de restriction sur
	// l'état de la vente) (like)
	...
	// select tous les articles vendus pour un utilisateur pour une catégorie pour
	// un article name
	public List<ArticleVendu> selectByUserByCategorieByArticleName(Utilisateur user, Categorie categorie,
	...
	public List<ArticleVendu> setEtatVente(List<ArticleVendu> articles) {
		for (ArticleVendu article : articlesVendu) {
			...
			if ((dateDebutEnchere.isBefore(LocalDate.now()) || dateDebutEnchere.equals(LocalDate.now()))
					&& (dateFinEnchere.isAfter(LocalDate.now()) || dateFinEnchere.equals(LocalDate.now()))) {
